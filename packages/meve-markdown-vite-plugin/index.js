...
function htmlWrapper(html) {
  ...
  const cardGroup = hGroup
    .map((fragment) => (fragment.includes('<h3') ? `<div class="meve-site-card">${fragment}</div>` : fragment))
    .join('')
...
function extractComponents(source) {
  ...
  source = source.replace(vueRE, (_, p1) => {
    ...
    return partComponents ? `<div class="meve-component-preview">${partComponents.join('\n')}</div>` : ''
  })
...
function injectCodeExample(source) {
  ...
  return source.replace(codeRE, '<app-code>$1</app-code>')
}
...
function markdownToVue(source, options) {
  ...
  return `
<template><div class="meve-site-doc">${templateString}</div></template>
...
function MeveMarkdownVitePlugin(options) {
  return {
...
module.exports = MeveMarkdownVitePlugin
